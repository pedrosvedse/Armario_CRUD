...
export default function EditarScreen({ route, navigation }) {
  ...
  useEffect(() => {
    api.get(`/armarios/${id}`).then(({ data }) => setForm({
      ...data,
      quantidadePortas: String(data.quantidadePortas),
...
const s = StyleSheet.create({
  ...
  btn: { backgroundColor: '#f39c12', padding: 14, borderRadius: 8, alignItems: 'center', marginTop: 24 },
